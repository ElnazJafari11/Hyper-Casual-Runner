...
|------|------------|--------|-------------------------------|
| **Kernel B** | Toolkit `IdleSliceState` (Melvor / Egg / Miner / …) | `IdleOfflineCatchUp` via `IdleSliceBootstrap.ApplyPersistedOfflineCatchUp` | Yes — on load after slice exists |
...
3. **`IdleSliceBootstrap.ApplyPersistedOfflineCatchUp`** stamps **after** `IdleOfflineCatchUp.Apply` when elapsed was consumed.
